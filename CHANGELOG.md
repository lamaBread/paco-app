...
### Fixed
- **`book_nl_chip()` 가 식별자를 빈 문자열로 흘리던 문제.** `nl_uri` 가 있어도
  `NlLod::bookResourceId()` 가 통제어휘 밖 URI 형태에서 `null` 을 반환하면 칩이 `NL `(식별자 없음)
  으로 렌더돼, *링크는 있는데 식별자 문자열만 사라지는* 현상이 났다. 추출 실패 시 URI 의 마지막 경로
  세그먼트를 폴백 라벨로 쓰도록 고쳐 식별자가 비지 않게 한다(`src/pages_common.php`).
- **`NlLod::bookResourceId()` URI 형태 확장.** `lod.nl.go.kr` 의 `/resource/` 뿐 아니라 `/page/`
  형태 자원 URI 에서도 식별자(KMO…·CNTS-…·WMO… 등)를 추출하도록 정규식을 넓혔다(`src/NlLod.php`).

### Changed
- **시집 국가서지LOD 칩을 시·비평문 화면에도 노출.** 그동안 시집 식별자 칩은 시집 목록·상세에서만
  보이고, 시 상세·비평문 상세에서는 그 시집이 명확한 국가서지LOD 링크를 가져도 표시되지 않았다(렌더
  비대칭). 시 상세의 *수록 시집* 과 비평문 상세의 *비평 대상* 에, 대상 시집이 `nl_uri` 를 가지면 칩을
  함께 보인다(`src/pages_common.php`·`src/pages_article.php`).

### 데이터 마이그레이션
- 없음(`SCHEMA_VERSION`=5 그대로). 표시·추출 로직 변경만.
